...
# CREATING THE QUERY WITH WIN AND LOSE ROWS
...
# WEIGHT OF EACH FEATURE IN THE MODEL DECISION
# TESTING SEED FEATURE
SELECT
  category,
  weight
FROM
...
# 39 ERROS AGAINST THE OVERALL OF 134 PREDICTIONS
SELECT * FROM `bracketology.predictions`
WHERE predicted_label <> label;

# ANALYSING THE ERROS:
# HIGH PROBABILITY OF LABEL (80%) BUT STILL GET IT WRONG
SELECT
...
# CREATING NEW MODEL WITH THE NEW FEATURES AND 
CREATE OR REPLACE MODEL
...
# EVALUATING METRICS OF THE NEW MODEL (5% BETTER THEN THE OLD ONE)
SELECT
  *
FROM
